...
var queryChaincode = async function(peer, channelName, chaincodeName, args, fcn, username, org_name) {
	...
	try {
		...
		if (response_payloads) {
			...
			return re.responseSuccess(response_payloads[0].toString('utf8'))
		} else {
			logger.error('response_payloads is null');
			return re.responseFail('response_payloads is null');
		}
	} catch(error) {
...
var getBlockByNumber = async function(peer, channelName, blockNumber, username, org_name) {
	try {
		...
		if (response_payload) {
			...
			return re.responseSuccess(response_payload);
		} else {
			logger.error('response_payload is null');
			return re.responseFail('response_payload is null');
		}
	} catch(error) {
		logger.error('Failed to query due to error: ' + error.stack ? error.stack : error);
		return re.responseFail(error.toString());
...
var getInstalledChaincodes = async function(peer, channelName, type, username, org_name) {
	try {
		...
		if (response) {
			...
			return re.responseSuccess(details);
		} else {
